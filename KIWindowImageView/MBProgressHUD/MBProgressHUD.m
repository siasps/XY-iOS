...
//图片轮播
+(id)showHUDAddedCustomViewTo:(UIView *)view animated:(BOOL)animated{
...
+(id)showHUDDetailTextTo:(UIView *)view animated:(BOOL)animated{
    MBProgressHUD *hud = [[self alloc] initWithView:view];
...
+(id)showHUDDetailTextToView:(UIView *)view text:(NSString *)text animated:(BOOL)animated{
    MBProgressHUD *hud = [[self alloc] initWithView:view];
    hud.mode = MBProgressHUDModeIndeterminate;
    hud.labelText = text;
    [view addSubview:hud];
...
//+(id)hideHUDCustomForView:(UIView *)view animated:(BOOL)animated
//{
//    MBProgressHUD *hud = [[self alloc] initWithView:view];
//    hud.mode = MBProgressHUDModeCustomView;
//    [hud hide:YES];
...
+(void)showAlterWithMessage:(NSString *)aTitle {
    [self showAlterWithMessage:aTitle afterDelay:1.0];
}

+(void)showAlterWithMessage:(NSString *)aTitle afterDelay:(CGFloat)afterDelay{
    if (aTitle.length <= 0) {
        return;
    }
    ...
    hud.margin = 10.0;
    hud.yOffset = 10.f;
    hud.size = CGSizeMake(hud.size.width, 50);
    hud.removeFromSuperViewOnHide = YES;
...
-(void)customImageView
{
    imageArray = [NSArray arrayWithObjects:
                  [UIImage imageNamed:@"加载中0.png"],
                  [UIImage imageNamed:@"加载中1.png"],
                  [UIImage imageNamed:@"加载中2.png"],
                  [UIImage imageNamed:@"加载中3.png"],
                  [UIImage imageNamed:@"加载中4.png"],
                  [UIImage imageNamed:@"加载中5.png"],nil];
    
//    CGSize totalSize = CGSizeZero;
//    totalSize.width += 2 * margin;
//    totalSize.height += 2 * margin;
//    CGFloat yPos = round(((self.bounds.size.height - totalSize.height) / 2)) + margin + yOffset;
    imageView = [[UIImageView alloc] initWithFrame:CGRectMake((SCREEN_WIDTH/2)-50,superBounds.size.height/2-50, 100, 100)];
    imageView.backgroundColor = [UIColor clearColor];
    //    imageView.image = [UIImage imageNamed:@"加载中1.png"];
...
- (void)drawRect:(CGRect)rect {
    ...
    //#warning：add by chen：2015-10-21
