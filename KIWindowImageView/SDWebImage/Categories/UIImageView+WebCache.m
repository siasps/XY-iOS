...
#warning --- modify by chen
                __block UIImage *newimage  = [UIImage compressionImage:image];
                
                if (newimage && (options & SDWebImageAvoidAutoSetImage) && completedBlock)
                {
                    completedBlock(newimage, error, cacheType, url);
                    return;
                }
                else if (newimage) {
